...
var questionData= require("../../../../page/common/data.js")
//console.log("22222222222222222222222="+questionData);
// const questionData = ""
var questionData;
//获取应用实例
const app = getApp()
Page({
  ...
  tapSelect: function (e) {
    ...
    if (bool) {
      ...
      var successNo = [];
      //将作对的数据进行保存
      successNo.push(this.data.swiper.active);

      this.data.answers.success++
    } else {
      this.data.answers.allList[this.data.answers.start + this.data.swiper.active].isAnswer = 2
      //统计错误数据,写入到本地，进行记录，并且错误记录仅保存一次，再次做对无效
      
      
      
      // errNo.push(this.data.swiper.active);
         
    //   //统计错误数据
    //   if(!isInArray(errNo,this.data.swiper.active)){
    //     var x = this.data.swiper.active;
    //     errNo.push(x);
    //     err.push(errNo);
    //   }
     
      this.data.answers.error++
      // wx.setStorageSync('countData', errNo);
    }
  ...
  onLoad: function (params) {
    ...
    if (app.globalData.userInfo) {
      ...
      // wx.request({
      //   url: app.globalData.ServerUrl+'exam/unit/'+wx.getStorageSync('current_lessonId)'),
